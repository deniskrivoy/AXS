...
async def create_request_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if saved_wallet:
        await callback.message.answer(
            ...
            f"👉 <b>Введите код сделки из бота Lolz Market OTC:</b>",
            parse_mode='HTML'
        )
...
async def finish_creation(message: types.Message, state: FSMContext):
    ...
    admin_text = (
        ...
        f"📎 <b>Доказательства:</b> {s1 and '✅' or '❌'} {s2 and '✅' or '❌'} {s3 and '✅' or '❌'}\n"
    )
...
# ================== АДМИН: ПОДТВЕРЖДЕНИЕ С ВЫПЛАТОЙ ==================
...
async def process_payout_amount(message: types.Message, state: FSMContext):
    ...
    # Отправляем воркеру уведомление о выплате с фото
    try:

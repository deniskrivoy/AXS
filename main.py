...
ADMIN_ID = 7652887576
SUPPORT_USERNAME = "LZT_Support_Official"  # изменено с TrueGuardSupport
...
async def start_cmd(message: types.Message):
    ...
    text = (
        ...
        "💳 Все выплаты производятся в <b>GRAM</b>.\n\n"
        "👇 Выберите действие:"
    )
...
async def create_request_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "3️⃣ Реквизиты для выплаты (кошелёк GRAM)\n"
...
async def process_deal_code(message: types.Message, state: FSMContext):
    ...
    # Убираем жесткую проверку на TF-XXXXXX, принимаем любой код
    if len(deal_code) < 3:
        await message.answer("❌ Слишком короткий код. Введите корректный код из бота Lolz Market OTC:", parse_mode='HTML')
        return
...
async def manual_pay(message: types.Message):
    ...
    try:
        ...
        await message.answer(f"✅ Выплата {amount} GRAM отправлена пользователю {username}.")
    except Exception as e:
